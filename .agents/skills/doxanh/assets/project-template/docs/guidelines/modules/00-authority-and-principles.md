...
> Canonical for: Generating, reviewing, implementing, verifying, and operating a project book for the selected web, API, and native surfaces
> Depends on: None
...
> Opinionated standard with independently selected Nuxt web, Nuxt/Nitro API,
> and Flutter native profiles.
>
> Baseline: Nuxt 4, Vue 3, TypeScript, shadcn-vue, Tailwind CSS, PostgreSQL,
> Drizzle ORM, Redis, BullMQ, GSAP, and Flutter. Install only the dependencies
> used by the selected profiles and active capabilities.

## 1. Purpose and authority

Select one or more implementation profiles explicitly; `shared` governance is
always active:

- **`nuxt-web`:** Nuxt browser UI and its rendering/application shell.
- **`nuxt-api`:** owned Nuxt/Nitro API services and their active data/process
  responsibilities, with no required frontend.
- **`flutter-native`:** installed native clients, independently or alongside web.

A full-stack web project selects `nuxt-web,nuxt-api`. A Flutter-only or web-only
client documents its actual API dependency and ownership without scaffolding a
backend, database, worker, or another client. Shared server/data requirements
govern an owned implementation or its documented integration contract; they do
not transfer ownership of an external system to this repository. A capability
is active only for its approved meaning and responsibility. Ask when profiles,
API availability, or ownership cannot be determined from project truth.
...
- **must** means release-blocking in its applicable profile and scope unless an
  ADR explicitly approves an exception
...
1. select the active web/API/Flutter profiles, implementation ownership, and capability decisions
...
- Follow the approved project design and comparable shipped screens first.
  Green primary and an administrative sidebar are fallback choices when no
  design exists, not mandatory replacements for approved branding or navigation.
  Keep one consistent shared design system; an already-approved design needs
  no new exception merely because it differs from those fallbacks.
