...
Apply each item to the declared phase, active profiles, owned capabilities, and
affected scope. These checklists reference acceptance criteria; they do not
expand GDL-080's test plan or require inactive services and clients. Initial
readiness approves contracts and exit criteria; implementation and release
evidence belongs to their later gates. Record N/A with the profile/ownership
reason, and ask about material unknowns instead of checking the box.
...
- [ ] The application master page follows the approved navigation and layout,
  defines scroll ownership/reset/restoration and phone/tablet/desktop behavior,
  and uses the shared sidebar/`ScrollArea` composition when that shell is selected.
...
- [ ] The project light/dark theme and shared design system are approved; green
  is used only as the fallback when no design exists.
...
- [ ] MVP phases and first vertical slice have approved exit criteria and evidence requirements.
...
- [ ] Every affected user-visible workflow has current focused platform proof:
  Playwright against the running application for web, and Flutter integration/
  native-device evidence for native UI. Use the real actor, isolated contexts,
  and rendered plus applicable durable/downstream outcomes; no lower-level or
  screenshot-only result substitutes for the claimed workflow boundary.
...
4. approved shared design direction and theme, light/dark behavior, and platform
   component mappings
...
6. selected Nuxt profiles: owned API composition and/or web scaffold, approved
   slot-based application layout and scroll ownership, Pinia blocking-activity controller and root
...
9. first end-to-end slice for each owned web/API surface, when active
10. when Flutter is selected, its independent or companion scaffold with `AppMasterPage`/`AppMasterLayout`,
   ...
   the documented API; native-only projects start here without a Nuxt scaffold
