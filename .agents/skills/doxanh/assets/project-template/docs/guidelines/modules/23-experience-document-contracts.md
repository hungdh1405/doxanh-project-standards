...
These contracts apply to each approved UI profile. An API-only project records
the fixed UI chapters as N/A and creates no fictitious screens. Native-only
projects use Flutter widgets, logical pixels, adaptive navigation, native scroll
ownership, `ThemeData`/`ColorScheme`, Dart validation, and platform accessibility
from GDL-053; web-only component names, CSS selectors, Zod form wiring, and
browser breakpoints are not native requirements. A combined product shares
product meaning and design roles while documenting each platform's realization.

Start from the project's approved design and comparable shipped screens. Record
their theme, navigation, density, component and layout decisions, and preserve
shared consistency. GDL-050's green theme and sidebar are defaults only when an
approved design is absent; they do not override an existing project design.
Link the existing approval instead of requiring a new ADR merely because its
colors or navigation differ from those defaults.
...
- master layout families, canonical master-page terminology, stable shell
  slots, approved navigation behavior (top bar/sidebar when present), the primary
  scroll owner, and the one global
  blocking-loading overlay
- page-template families and copyable template checklists
- product-pattern-to-shadcn-vue mapping for web and native widget mapping for Flutter
...
- `:root` and `.dark` semantic mappings for web; light/dark
  `ThemeData`/`ColorScheme` and theme-extension mappings for Flutter
- approved primary and primary-foreground pairs; green is the fallback only
  when the project has no approved theme
...
It must not restyle default shadcn-vue components or shared native widgets page
by page.
...
- approved shell/navigation/footer slots, top bar/sidebar only when present,
  primary web `ScrollArea` or
  native scroll owner, scroll
...
Each `docs/mock-ui/screens/<screen-code>-<slug>.md` must use the Section 8.9
contract structure for web or its GDL-053 native adaptation and contain:
...
For a consumed external API with different result semantics, reference its
approved typed contract or explicit adapter; do not invent code-0/non-zero
provider behavior to fill a matrix heading.
...
| Viewport/mode | Composition and reading order | Approved navigation/actions | Moved/condensed/hidden information | Scroll owner/overflow/sticky behavior | Touch/keyboard behavior | Evidence |
| --- | --- | --- | --- | --- | --- | --- |
