...
Apply each contract to the processes, data, transports, and clients the project
actually owns. `nuxt-api` owns the Nuxt/Nitro server implementation baseline;
`nuxt-web` and `flutter-native` may instead consume a documented external API.
Describe external contracts and trust boundaries without claiming ownership of
their server, database, activity store, or deployment. PostgreSQL/Drizzle and
server-process details below apply to the owned API/data profile. A native local
store documents its actual schema, retention, migrations, and recovery in the
mobile contract; it does not acquire a fictional PostgreSQL backend.

`system-architecture.md` must include:

- system context and runtime/container views
- active web, API, worker, scheduler, migration, realtime gateway, and
  native-client processes, with external services explicitly distinguished
...
`api-contract.md` must identify the owning service and whether each endpoint is
implemented here or consumed externally, then include an endpoint catalog:

| ID | Method | Path | Caller | Auth/scope | Permission/state | Request schema | Success data | Handled outcomes | Unexpected failure | Idempotency/concurrency | Owner |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
Owned first-party application endpoints adopt the mandatory envelope from
Section 10.1; do not invent a project-specific top-level alternative. For a
consumed external API, preserve its actual approved contract and document any
explicit client adapter rather than pretending the provider uses this envelope.
Define request IDs, pagination, filtering, sorting,
files, streams, webhooks, versioning, and every exception to the normal JSON
shape.

When external integrations or webhooks are active, this chapter also owns their
catalog and the project-specific decisions required by Section 10.1.4:

| Integration | Provider/version/environment | Official contract source | Incoming/outgoing operation | Auth/signature and scope | Schema/adapter owner | Replay/idempotency and durable handoff | Limits/retry/reconciliation | Tests and evidence |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |

Link to the canonical product, security, data, job, and operational consumers;
do not create a competing generic integration chapter. Active file APIs also
map upload, validation, publication, access, retention, and deletion ownership
to the lifecycle contract in Section 10.9.

Maintain one result-code registry for owned Doxanh API/command outcomes. A
consumed external API documents its actual status/error semantics without
inventing an incompatible numeric code registry:
...
Every project owning a server-backed durable business/audit activity store must define a first-class,
...
| `result_code` | Required non-null numeric result code from the API, command, or registered activity outcome. Use `0` only for true success and a registered non-zero code for every other outcome, including non-HTTP producers. |
...
When realtime is active, `realtime-protocol.md` must define the approved
transport and its supported client contract instead of scattering it across
source comments. SSE may serve one-way browser updates; the WebSocket topology,
ticket, frame, and gateway requirements below apply when bidirectional WebSocket
transport is selected under GDL-066. Request-driven polling remains an inactive
realtime decision. Do not generate a Flutter socket, WebSocket gateway, or Redis
fanout merely to document an SSE feed.

An SSE contract names its endpoint, `text/event-stream` content type and HTTP
behavior, session/authentication and origin/CORS policy, stream authorization
and revocation, event schema, heartbeat, limits/backpressure, disconnect and
reconnect behavior, and error/recovery evidence. Use `Last-Event-ID` replay only
when a durable event store and cursor contract exist; otherwise refetch
authoritative state after reconnect. General authorization, durable truth,
redaction, bounded delivery, and recovery requirements apply to every active
transport.

For the WebSocket profile, document:
...
1. scope and capability decision:
   ...
   - approved standards-based WebSocket baseline and the dated library
     acceptance evidence from Sections 3.7 and 3.8
2. topology:
...
8. client behavior, only for approved supported clients:
