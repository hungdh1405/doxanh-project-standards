...
First identify independently owned `nuxt-web`, `nuxt-api`, and `flutter-native`
profiles, external service contracts, and supported targets. Apply UI design,
server/data architecture, native integration, and delivery steps only to those
owners. Ask before assuming a missing backend or web companion.
...
- Reuse the project's approved design direction and comparable shipped screens.
  Generate and review a product-specific direction only when one is missing;
  do not accept a generic SaaS or AI visual default.
- Persist the approved master design system.
- Define the approved project's primary tokens, navigation and light/dark
  mappings. Green and sidebar are baseline defaults only when an approved
  design is absent; differing approved colors or navigation need no new ADR.
...
Build slices across every applicable boundary below. A UI-only or API-only
change does not need invented server work, screens, infrastructure, or tests to
fill this list. Select verification through GDL-080 and reuse current evidence:
...
Use framework conventions before inventing folders. The combined example below
shows independently selectable web and API responsibilities: `app/` and browser
assets belong to `nuxt-web`; `server/`, Drizzle, workers, and owned service
infrastructure belong to `nuxt-api` and its active capabilities. A web client
using an external API does not scaffold those backend paths. Flutter follows
GDL-053 in its own runnable workspace and requires none of these Nuxt files.
...
│   │   ├── common/
│   │   │   ├── shell/          # AppMasterLayout/AppLoadingOverlay and approved navigation
...
│   │   ├── client.ts
│   │   └── schema/
│   ├── queues/                 # only when queue work is active
│   │   ├── config.ts
│   │   ├── names.ts
│   │   ├── identity.ts
│   │   ├── envelope.ts
...
│   ├── fanout/
│   └── utils/
├── workers/                    # only when queue/scheduler work is active
│   ├── index.ts
│   ├── processors/
│   ├── schedulers/
│   └── utils/
...
Keep active `server/`, `shared/`, `workers/`, `public/`, `nuxt.config.ts`, and
`drizzle.config.ts` paths at the repository root; omit conditional directories
when their capability is inactive. Keep browser application code under
...
When Flutter is in scope, it may be the sole application in its repository or
live in an explicit workspace such as `apps/mobile/`. In a monorepo, give each
owned Nuxt web/API application an explicit workspace and document the changed
root paths. Do not create another platform merely to match the example layout.
...
- The documented owning API service remains authoritative for validation,
  authorization, business state, and persistence. It may be the project's
  `nuxt-api` workspace or an approved external service.
