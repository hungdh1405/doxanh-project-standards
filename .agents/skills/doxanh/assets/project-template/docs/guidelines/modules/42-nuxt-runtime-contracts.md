...
blocking rule for an active Nuxt web/API profile that renders, accepts or
persists temporal values. Select the responsibilities the project owns;
client rendering requirements are not prerequisites for an API-only service.

Server endpoint, validation and persistence requirements below apply to an owned
API. A client using an external API follows its approved wire contract through
the documented adapter from Section 10.1; it does not invent response fields or
rewrite the remote service. Resolve missing timezone, scope or format decisions
before implementing the dependent presentation. Pinia/SSR rules apply to the
Nuxt web client; the native realization is in Section 9.6.2.
...
This section applies only to the active `nuxt-web` profile.
...
- covers the application viewport, including the approved primary scroll region
...
- prevents keyboard, shortcut, and assistive-technology activation of blocked
  content as well; `aria-busy` and a pointer backdrop alone do not disable it.
  Keep the status/recovery region outside any inert subtree and restore useful
  focus after release, accounting for navigation or a removed trigger
...
Apply TypeScript and public-boundary rules to owned web/API TypeScript source.
Vue, shared client catalogs and Vite rules apply only when those consumers or
build paths exist; an API-only service does not scaffold them.
