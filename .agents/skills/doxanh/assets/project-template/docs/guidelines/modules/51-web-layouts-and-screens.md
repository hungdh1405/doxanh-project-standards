...
The approved project design selects navigation, chrome and primary scroll
ownership. Reuse it without a new ADR. When those decisions are missing, use
the bounded administrative shell described below with a top bar, shadcn-vue
`Sidebar`/`SidebarTrigger` and main `ScrollArea`. Sections 8.7.1–8.7.3 describe
that default realization; an approved alternative keeps the same shared
ownership, responsive usability, focus and restoration obligations through its
...
- **Page template** is the reusable content structure rendered through the
  master layout's content slot and approved primary scroll owner.
...
Every authenticated routed page uses its approved shared layout. When the
default administrative shell is selected, it has:
...
Public, authentication and other approved layout families use their documented
navigation semantics. A fullscreen
...
When using the default administrative shell, use this shadcn-vue composition:
...
For this bounded-shell realization, the master layout creates the primary
`ScrollArea`. Pages
...
Record the approved primary scroll owner for each layout in `ui-system.md`.
Native document scrolling or another approved shell is a normal design decision
with explicit route reset, restoration and accessibility behavior. When no
decision exists, default application pages to the bounded main `ScrollArea`.
Later screen/template references to `ScrollArea` describe that realization;
use the approved owner's equivalent behavior for another selected shell.
...
Use this contract structure for every important web screen. Remove an optional
section only when the approved template permits it; otherwise write
`N/A — <reason>`.

For a native screen, retain the metadata and meaningful content, action, data,
state, permission, accessibility and evidence sections, and use Section 9's
native realization: typed native routes/deep links, widget/scroll/navigation
owners, logical-pixel constraints, native validators and Flutter/device test
evidence. Replace the web-only `Sidebar`, `ScrollArea`, shadcn-vue `Pagination`,
CSS zoom and Zod client-field rows with their applicable native contracts; do
not copy those web implementation names into a Flutter project. An API-only
project with no owned UI does not generate screen contracts.
...
```text
Show the approved shell, responsive navigation state, primary scroll boundary,
heading, content order, real copy, actions, pagination,
...
- Approved primary scroll owner's result-reset and back/forward restoration behavior:
...
- Approved navigation open/close focus, route focus, and scroll reset/restoration:
