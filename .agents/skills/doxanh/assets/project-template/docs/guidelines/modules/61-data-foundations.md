...
Apply these implementation rules when the project owns persisted server data.
A client consuming an external API records that system's authoritative data
boundary and its exposed guarantees; it does not create a local PostgreSQL or
Drizzle backend merely to satisfy this chapter.
