...
Apply this subsection only to active payment behavior. Destination management,
provider catalogs, QR encoding, billing, and printing are separate decisions;
activate only the parts the approved workflow uses. A read-only payment-history
screen does not require editable receiving accounts, and printing a
nonfinancial document does not activate payments. Record unused sub-capabilities
as `N/A` with their activation trigger.
...
- Materialize these decisions through product/domain requirements, API Zod
  ...
  account format, settlement authority, or provider behavior is unclear, block
  the dependent slice and ask the product owner under Section 4.18. Continue
  independent documentation and work that does not depend on the answer.

Financial settlement, adjustment, and printing decisions:

The billing and settlement rules below apply only when the application owns
those financial operations. Adjustment, refund, split-settlement, and unpaid
outcome rules require their corresponding approved workflows; do not invent
those features to satisfy an example.
...
- Snapshot applicable immutable line, adjustment, tax, total, destination, protocol
  payload, actor, and time facts at the documented financial-finalization
  boundary. Historical bills and reports must not read mutable item, provider,
  account, display-name, or tax configuration as their source of truth.
- Require row locks or optimistic versions, a durable idempotency key, database
  constraints matching the approved full/partial settlement and finalization
  policy, same-transaction financial activity, safe
  committed invalidation events, and concurrent same-key/different-key tests.

Printing when active:

Apply the selected output path only: browser/PDF, native system print, or an
approved bridge/device integration. The browser-specific layout and evidence
rules below do not mandate browser printing for a native client. Payment
content and settlement snapshots apply only to financial documents.

- Decide printing topology separately from business-document content. Browser print preview
  is a manual client-side output and must render only an authorized,
  server-resolved document revision through a dedicated layout; terminal receipts
