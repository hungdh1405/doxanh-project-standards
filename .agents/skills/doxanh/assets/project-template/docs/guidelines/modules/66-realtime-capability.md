...
Choose and document the transport before applying its topology. Polling remains
request-driven and does not activate a realtime gateway. The WebSocket
libraries, ticket/frame protocol, dedicated gateway/Redis, and cross-replica
proof below apply to the selected WebSocket profile. Cover only clients and
deployment roles actually supported by the project; a web-only feed does not
require Flutter.

For an approved one-way browser SSE feed, define the `text/event-stream`
endpoint, authentication and origin/CORS policy, scope-safe event projection,
access revocation, heartbeat/idle timeout, reconnect, resource limits, and
operational ownership. Use the event's durable meaning from
`docs/realtime-events.md`. An event ID or `Last-Event-ID` header promises replay
only when the server implements an authorized retained cursor; otherwise
reconnect and refetch authoritative state. Do not add WebSocket tickets, binary
frames, a Flutter socket client, or a separate Redis/gateway role merely to
satisfy this module. Authorization, bounded delivery, safe telemetry, and honest
loss/recovery semantics apply to either live transport.
...
- Nitro-native WebSocket support or a cross-runtime wrapper may replace the
  ...
  target, and load evidence pass Sections 3.7 and 3.8. Do not base a generic production
...
- Load-test representative supported-client connection churn, steady
