...
Provide the signals for active process roles and dependencies. Cache, queue,
scheduler, realtime, native, and provider metrics below are conditional; their
presence in this list is not a requirement to install those capabilities.
Readiness checks distinguish required dependencies from optional/degradable
ones using the documented failure policy.
