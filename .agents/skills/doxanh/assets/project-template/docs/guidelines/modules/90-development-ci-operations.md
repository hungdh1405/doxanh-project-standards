...
Docker Compose is the required local-development environment for owned Nuxt
web/API services. A developer
...
| --- | --- | --- |
| `web` / `api` | Required for the corresponding owned Nuxt profile | Nuxt development server; binds inside the container to `0.0.0.0`; exposes the documented host URL. A full-stack process may serve both. |
| `postgres` | Required when this project owns PostgreSQL persistence | Project database with healthcheck and project-scoped named volume. |
...
| `migrate` | Required when this project owns database migrations | Applies committed Drizzle migrations deliberately; it is not an always-running service or per-replica startup hook. |
...
Apply each target to its owned profile/capability. An inactive database, queue,
browser, or service is documented as N/A; it does not require a dummy target or
new infrastructure. Native build/device commands use the supported host tools.
...
| `make test-api` / `make test-e2e-smoke` / `make test-e2e` / `make test-a11y` | Run the applicable canonical API, browser, and accessibility scripts for the selected scope and GDL-080 browser policy. |
| `make docs-check` | Run the complete project-book validation command family. |
| `make verify-automated` | From a clean immutable checkout, run only missing automated checks selected by the release plan and its universal baseline. |
...
| `make verify` | Aggregate current plan-selected automated, manual, and deployed-target evidence for the same candidate without rerunning suites. |
...
CI uses `VERIFY-SCOPE-001`, not an unconditional copy of this coverage catalog.
Pull-request jobs compute the complete merge-base-to-candidate diff; push jobs
use the recorded accepted comparison base. Include maintained worktree changes
when present. A clean checkout is not an empty change: never compare only the
worktree with `HEAD`. Missing/shallow base history must be fetched or reported
as an unresolved scope; it cannot produce a false passing no-op. Validate the
planner with a clean committed feature change as well as a dirty local change.
Required repository integrity gates still run; application suites use the union
of affected profiles and boundaries. Releases add the phase-aware baseline;
full regression requires an explicit recorded trigger.

Maintain the following coverage for active profiles/capabilities and dispatch
only the checks selected by that plan:
