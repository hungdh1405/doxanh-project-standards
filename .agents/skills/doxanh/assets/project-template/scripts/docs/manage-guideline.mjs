...
for (const [profile, rules] of Object.entries(manifest.task_reading?.profile_rules ?? {})) {
  if (![...manifest.profiles.required, ...manifest.profiles.optional].includes(profile)) manifestErrors.push(`unknown task profile ${profile}`)
  if (!isRecord(rules)) { manifestErrors.push(`${profile}: rule mapping must be an object`); continue }
  for (const [rule, ids] of Object.entries(rules)) {
    if (!Object.hasOwn(manifest.task_reading.rules, rule)) manifestErrors.push(`unknown profile rule ${rule}`)
    for (const id of requireStringArray(manifestErrors, `${profile}.${rule}`, ids)) {
      if (!knownModuleIds.has(id)) manifestErrors.push(`unknown task module ${id}`)
    }
  }
}
...
function validateUiCopyAndDensityContracts(source) {
  ...
  if (copyStart < 0 || copyEnd < 0) {
    ...
    for (const token of [
      ...
      'review every affected rendered title',
    ]) {
...
async function plan() {
  ...
  const taskProfiles = parseCsvOption('--task-profiles')
  ...
  if (mode === 'project' && !manifest.profiles.optional.some(profile => profiles.has(profile))) {
    fail('project mode requires approved --profiles: nuxt-web, nuxt-api, flutter-native, or a combination; ask when unknown')
  }
  for (const profile of taskProfiles) {
    if (!profiles.has(profile)) fail(`task profile ${profile} is not an approved project profile`)
  }
  if (!taskProfiles.size) profiles.forEach(profile => taskProfiles.add(profile))
  manifest.profiles.required.forEach(profile => taskProfiles.add(profile))
  for (const rule of rules) {
    ...
    const owners = new Set(manifest.task_reading.rules[rule])
    for (const profile of taskProfiles) {
      for (const id of manifest.task_reading.profile_rules[profile]?.[rule] ?? []) owners.add(id)
    }
    if (!owners.size) fail(`${rule} requires an applicable approved task profile; ask when unknown`)
    owners.forEach(id => requestedModules.add(id))
  }
  ...
  const available = manifest.modules.filter(
    module => isActive(module, mode === 'task' ? taskProfiles : profiles, capabilities),
  )
  if (mode === 'task') {
  ...
  console.log(JSON.stringify({
    ...
    ...(mode === 'task' ? { task_profiles: [...taskProfiles].sort() } : {}),
