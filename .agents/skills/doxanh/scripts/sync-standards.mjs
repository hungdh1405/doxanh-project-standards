...
try {
  ...
  await mkdir(guard)
  destination = resolve(home, 'doxanh')
  const alreadyLinked = run('manage-user-skill.mjs', 'check', true)
  if (!alreadyLinked && await exists(destination)) {
    backup = resolve(home, `.doxanh.sync-${process.pid}-${Date.now()}.bak`)
    await rename(destination, backup)
  }
  try {
    legacy = resolve(home, 'project-guideline-workflow')
    if (await exists(legacy)) {
      legacyBackup = resolve(home, `.project-guideline-workflow.sync-${process.pid}-${Date.now()}.bak`)
    ...
    if (legacyBackup) console.log(`Previous legacy skill retained at ${legacyBackup}`)
  }
  catch (error) {
