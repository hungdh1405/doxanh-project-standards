...
| `docs/local-development.md` | Required for the Nuxt profile | Docker/Compose topology, setup, Make targets, ports, networks, volumes, seeds, reset safety, and troubleshooting. |
| `docs/api-contract.md` | Required for server/API work | Endpoints, auth, request/response schemas, errors, pagination, idempotency, and examples. |
| `docs/database-schema.md` | Required for persisted data | Physical data model, fields, relationships, tenant integrity, lifecycle, transaction/query paths, constraints, indexes, roles/RLS, migrations, scale, and evidence. |
| `docs/activity-log.md` | Required | Durable activity/audit semantics, module and tag registry, actor/source/target context, access, retention, and verification. |
...
| `docs/design-tokens.md` | Required for UI | Canonical theme tokens, green primary choice, type, spacing, widths, motion, and accessibility pairs. |
...
Every generated Nuxt project must also expose the documentation command
...
High-risk requirements must trace from product rule to UI, server enforcement,
database state, registered activity tag/durable row, and automated proof.
...
book:
  ...
  profiles:
    - nuxt-web
    # Add flutter-native only when approved.
  default_locale: en
...
- `profiles`: `shared`, `nuxt-web`, or `flutter-native`
...
- Conditional chapters still have a real file. When inactive, use
...
- Flutter development: web contracts plus `37` and `41`, and `38` when realtime is active.
