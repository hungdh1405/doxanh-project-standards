...
## 9. Flutter native-mobile companion standard

### 9.1 Scope and platform boundary

Use Flutter only when the approved product scope includes an installed native
mobile application. A responsive web experience remains mandatory for the Nuxt
profile unless the product specification explicitly excludes it.
...
| Theme | Material 3 `ThemeData` and `ColorScheme.fromSeed` | Use the green seed/primary with light, dark, and system modes. Do not require a theme package for behavior available in the SDK. |
...
- Use default Material 3 widgets and platform conventions before custom
  controls.
- Use the green theme seed/primary and semantic theme tokens; do not recolor
  screens independently.
- Keep density compact but preserve a minimum 48×48 logical-pixel interactive
  target.
- Reuse the master layouts and page-template intent from Section 8, translating
  them into native screens, sheets, dialogs, side navigation, and platform
  navigation patterns.
...
Test at the appropriate boundaries:
...
Every Flutter project documents equivalent commands for:
...
Run only the release builds for approved target platforms. CI must verify code
generation drift, formatting, analysis, unit/widget tests, applicable
integration tests, and at least one release-mode build per shipping platform.
...
- [ ] Green primary and semantic colors work in light, dark, and system modes.
